...
namespace AcumaticaFilesImport.Acumatica
{
    public class AcumaticaWorker : IDisposable
    {
        ...
        public void AttachToRecord(UploadItem item)
        {
            ...
            {
                FileStatus = FileStatus.UnSupportedException;
            ...
            catch (ApiException a)
            {
                ...
                {
                    FileStatus = FileStatus.LogRecordMissing;
                    _logger.LogRecordMissing(item);
                }
                else
                {
                    FileStatus = FileStatus.Exception;
        ...
        private Action<IEnumerable<string>, string, byte[]> GetAttachFunction(DocType docType)
        {
            ...
            switch (docType)
            {
                ...
                case DocType.ARInvoice:
                    ...
                    break;
                case DocType.ARPayment:
                    action = GetApi<PaymentApi, Payment>().PutFile;
                    break;
                case DocType.APPayment:
                    action = GetApi<PaymentApi, Payment>().PutFile;
                    break;
                case DocType.PurchaseOrder:
                    action = GetApi<PurchaseOrderApi, PurchaseOrder>().PutFile;
                    break;
                case DocType.APInvoice:
                    action = GetApi<InvoiceApi, Invoice>().PutFile;
                    break;
                case DocType.StockItem:
                    action = GetApi<StockItemApi, StockItem>().PutFile;
                    break;
                default:
